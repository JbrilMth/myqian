...
import { Plus, Tag, AlertCircle, Check, Loader2 } from "lucide-react";
import type {
  AttendanceCategory,
...
interface EditAttendanceModalProps {
  isOpen: boolean;
  onClose: () => void;
  record: AttendanceRecordWithCategory | null;
  categories: AttendanceCategory[];
  onSuccess: (updated: AttendanceRecordWithCategory) => void;
}
...
export function EditAttendanceModal({
  ...
  const prevRecordIdRef = useRef<string | null | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const isRecordChanging = isOpen && record?.id !== prevRecordIdRef.current;

    if (isOpening || isRecordChanging) {
      if (record) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4 pt-2">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., Office Work, Study Session"
